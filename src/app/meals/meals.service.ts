...
export class MealsService {
  ...
  private selectedElementSubject = new BehaviorSubject<MealElement | null>(
    null
  );
  ...
  updateMeal(meal: MealElement): Observable<any> {
    this.loadingSubject.next(true);
    const currentMeals = this.originalElementDataSubject.value;
    const updatedMeals = currentMeals.map((item) =>
      item.name === meal.name ? meal : item
    );
    return this.updateMealsOnServer(updatedMeals).pipe(
      tap({
        next: () => {
    ...
    return this.http.put(url, meals).pipe(
      tap(() => {
        ...
        if (this.currentFilterOptions) {
          const filteredMeals = this.applyFilter(
            meals,
  ...
  private applyFilter(
    elements: MealElement[],
    filterOptions: any
  ): MealElement[] {
    return elements.filter((element) => {
      const caloriesCondition =
        (filterOptions.calories === 'small' && element.calories < 300) ||
        (filterOptions.calories === 'medium' &&
          element.calories >= 300 &&
          element.calories <= 700) ||
        (filterOptions.calories === 'large' && element.calories > 700) ||
        !filterOptions.calories;
      const kindCondition =
        !filterOptions.kind || element.kind === filterOptions.kind;
      const tasteCondition =
        !filterOptions.taste || element.taste === filterOptions.taste;
